...
if (!empty($_POST)) {
	...
	if ($operacion == 'alta' && !isset($_GET['id_pers'])) {
		...
		$persona->guardar();
	} else if ($operacion == 'actualizar' && isset($_GET['id_pers'])) {
		$persona->nombre = $_POST['txtNombre'];
		$persona->apellido = $_POST['txtApellido'];
		$persona->sexo = $_POST['txtSexo'];
		$persona->dni = $_POST['txtDni'];
	...
	} else if ($operacion == 'borrar' && isset($_GET['id_pers'])) {
		$persona->borrar($_GET['id_pers']);
	} else if ($operacion == 'edicion' && isset($_GET['id_usuario'])) {
...
<script src="../public/js/funciones_p.js"></script>
...
<div class="container-fluid">
	<nav class="navbar navbar-default " role="navigation">
		...
		<ul class="nav navbar-nav" style="padding-top: 10px;padding-bottom: 0px;">
			...
			<button type="button" class="btn btn-primary  btn-sm" onclick="cargar('#capa_d','../models/Persona/alta_p.php')">Alta</button>
		</ul>
